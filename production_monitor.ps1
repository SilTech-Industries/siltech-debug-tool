# SilTech Industries - Production Serial Monitor
# Simplified for factory use: auto-connects, Q to quit
# Called by production_flash.bat after flashing
#
# Usage: production_monitor.ps1 -ComPort COM3
...
# --- Wait for device to boot after hard reset ---
Write-Host "  Waiting for device to boot..." -ForegroundColor DarkGray
Start-Sleep -Seconds 2

# --- Log setup ---
if (-not (Test-Path $LOG_DIR)) { New-Item -ItemType Directory -Path $LOG_DIR | Out-Null }
$logFile = Join-Path $LOG_DIR ("production_" + (Get-Date -Format "yyyy-MM-dd") + ".log")
...
Write-Host "   Press Q + Enter to stop and flash next device" -ForegroundColor Yellow
Write-Host "  ================================================" -ForegroundColor Green
Write-Host ""

Add-Content -Path $logFile -Value "`n=== Monitor started $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') on $ComPort ==="
...
while ($running -and $port.IsOpen) {
    ...
    if ($port.BytesToRead -gt 0) {
        ...
        if ($data) {
            ...
            foreach ($rawLine in $lines) {
                ...
                } elseif ($line -match "WiFi|MQTT|connected|IP:|OTA|version") {
    ...
    while ([Console]::KeyAvailable) {
        ...
        if ($key.Key -eq "Enter") {
            ...
            # Send any typed command to device
...
$port.Dispose()
Add-Content -Path $logFile -Value "=== Monitor stopped $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ($lineCount lines) ==="
Write-Host ""
Write-Host "  Monitor stopped. ($lineCount lines captured)" -ForegroundColor Cyan
Write-Host ""
